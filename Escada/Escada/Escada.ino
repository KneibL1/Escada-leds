...
#define NUM_LEDS_PER_STRIP 10   // Número de LEDs por fita
...
CRGB currentColor[NUM_STRIPS];
unsigned long lastUpdate = 0;
unsigned long lastSensorCheck = 0;
bool escadaLigada = false; // Estado da escada
...
void setup() {
  ...
  for (int i = 0; i < NUM_STRIPS; i++) {
    ...
    currentColor[i] = CRGB::White;  // Cor padrão
  }
...
void loop() {
  ...
  if (escadaLigada) {
    ...
    // Se a escada estiver desligada, todos os LEDs devem estar apagados
    for (int i = 0; i < NUM_STRIPS; i++) {
      currentBrightness[i] = BRIGHTNESS_LOW;
      FastLED.setBrightness(BRIGHTNESS_LOW);
      for (int j = 0; j < NUM_LEDS_PER_STRIP; j++) {
...
void processarComando(String comando) {
  comando.trim();
  if (comando == "ligar escada") {
    escadaLigada = true;
  } else if (comando == "desligar escada") {
    escadaLigada = false;
  } else if (comando.startsWith("escada cor")) {
    String cor = comando.substring(comando.indexOf(" ") + 1);
    CRGB novaCor = stringParaCor(cor);
    for (int i = 0; i < NUM_STRIPS; i++) {
      currentColor[i] = novaCor;
    }
    FastLED.show();
  } else if (comando.startsWith("escada brilho")) {
    int brilho = comando.substring(comando.indexOf(" ") + 1).toInt();
    if (brilho >= 1 && brilho <= 100) {
      int brightness = map(brilho, 1, 100, BRIGHTNESS_LOW, BRIGHTNESS_HIGH);
      for (int i = 0; i < NUM_STRIPS; i++) {
...
CRGB stringParaCor(String cor) {
  cor.toLowerCase();
  if (cor == "vermelho") return CRGB::Red;
  if (cor == "verde") return CRGB::Green;
  if (cor == "azul") return CRGB::Blue;
  if (cor == "branco") return CRGB::White;
  if (cor == "amarelo") return CRGB::Yellow;
  if (cor == "ciano") return CRGB::Cyan;
  if (cor == "magenta") return CRGB::Magenta;
  return CRGB::White; // Cor padrão se não reconhecida
}
